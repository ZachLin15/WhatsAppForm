...
logging.basicConfig(filename=r'C:\Users\USER\ImportOracle\pythonProject1\Log\Simplr_WS2_Import.log',level=logging.INFO,
...
exceldata = r"C:\Users\USER\ImportOracle\pythonProject1\dist\data2.xlsx"

def GetLastestCustomer(exceldata):
    url = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQWg0_amkjWdGG_DPiNMy4Udd0_xJBgxtydB5bywd72ocfKAyDlKLvx12xTay5yo0yX8pxKCCuB2YGS/pub?output=xlsx"
...
def Output(sheet,combine_df):
    ...
    with open(r"C:\Users\USER\ImportOracle\pythonProject1\dist\data2.xlsx", "wb") as file:
        file.write(response.content)
    ...
    #data['delivery_date_required'] = data[data['delivery_date_supplier'] >= today.copy()
    ...
    data["po_no"] = data.apply(
        lambda row: row["po_no"] + "-F" if row["supplier_item_code"].startswith(
            ("FR", "ZF", "FSI", "FF", "ZKF", "RMFR","RMVE" ,"CH","UN","JOFRBT")) else row["po_no"] + "-D",
        axis=1)
...
def Export_Query(file_path,receiver_email):
        ...
        try:
            ...
            logging.info(f" {path} Import Successul")
            now = datetime.now()
