...
logging.basicConfig(filename=r'C:\Users\USER\ImportOracle\pythonProject1\Simplr_WS5_Import.log',level=logging.INFO,
...
def Output(sheet,combine_df):
    ...
    with open("data5.xlsx", "wb") as file:
        file.write(response.content)


    # Load the Excel file and select the "Angus Steak House" sheet
    data = pd.read_excel("data5.xlsx", sheet_name=sheet)
    ...
    data["po_no"] = data.apply(
        lambda row: row["po_no"] + "-F" if row["item_name"].startswith(("FR", "ZF", "FSI")) else row["po_no"] + "-D",
        axis=1)
    ...
    data['supplier_item_code'] = data['supplier_item_code'].str.strip().str[:20]
...
if __name__ == '__main__':
    ...
    form = ['Angus Steak House', 'Biz IQ Academy', 'Bread Butter Jam', 'Singapore Food Delight', 'Yong Zhan',
             'Cheval Group', 'The Harvest', 'Pasta Craft','Windowsill Pies','Perfect Choice Investment','Kenny Rogers',
            'Jacobs Cafe', 'Caffe Beviamo', 'Fraser Suites', 'Xin Ju Feng', 'Thai Baang']
